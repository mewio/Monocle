...
def _format_seconds(seconds):
    """Format a number of seconds as a string.
    """
    parts = []
...
class _EventFormatter(string.Formatter):
    """Formatter with missing-field fallback text and field use tracking.
    """
    def __init__(self):
        self.used_args = set()
    ...
    def convert_field(self, value, conversion):
        "Prevent conversions from being applied to the sentinel value."
...
class DiscordSender(Sender):
    ...
    def have_global_config():
        ...
        """
        return bool(conf.DISCORD_WEBHOOK_ID and conf.DISCORD_WEBHOOK_TOKEN)

    def __init__(self, *, loop, websession, config=None):

        super().__init__(loop=loop, websession=websession, config=config)

        self._url = '/'.join(('https://discordapp.com/api/webhooks',
            self.config['WEBHOOK_ID'], self.config['WEBHOOK_TOKEN']))
    ...
    async def _call(self, method, data=None):
        ...
        """
        try:
            async with self.websession.request(method, self._url, json=data):
                pass
    ...
    def _sanitize_err_args(self, err):
        ...
        """
        for arg in err.args:
            if isinstance(arg, str) and self.config['WEBHOOK_TOKEN'] in arg:
                yield arg.replace(self.config['WEBHOOK_TOKEN'], "<TOKEN>")
            else:
                yield arg
    ...
    def _make_spawn_data(spawn):
        ...
        if conf.DISCORD_SPAWN_DATA:
            hookdata = conf.DISCORD_SPAWN_DATA
        else:
            hookdata = dict(
                ...
                embeds=[
                    dict(
                        ...
                        image=dict(url=conf.DISCORD_SPAWN_EMBED_MAP),
                        ),
                    ],
                )
    ...
    def __str__(self):
        return "Discord webhook id {}".format(self.config['WEBHOOK_ID'])
...
class PushbulletSender(Sender):
    ...
    async def send(self, event, attachments=()):
        ...
        try:
            templates = self._MESSAGE_TEMPLATES[type(event)]
    ...
    # Map event type to text templates
    _MESSAGE_TEMPLATES = {
        Spawn: _TemplateGroup(
...
class SMTPSender(Sender):
    ...
    # Map event type to email message templates
    _MESSAGE_TEMPLATES = {
        Spawn: _TemplateGroup(
...
class TelegramSender(Sender):
    ...
    async def send(self, event, attachments=()):
        """Send an event notification, ignoring attachments.
        """
        method, data = self._MESSAGE_MAKERS[type(event)](event)
    ...
    @staticmethod
    def _make_spawn_data(spawn):
        """Return method name & data suitable for reporting a Spawn event.
        """
        return 'sendVenue', dict(
            latitude=spawn.latitude,
            longitude=spawn.longitude,
            title=spawn.format(conf.TELEGRAM_SPAWN_TITLE),
            address=spawn.format(conf.TELEGRAM_SPAWN_TEXT),
            )
...
def _separate_twitter_spawn_formats():
    """Separate twitter spawn formats with exact times from those with ranges.

    Keeping exact-time templates separate from time-range templates helps us
    quickly choose the most appropriate one for any given spawn depending on
    whether the exact despawn time is known.

    :return:            A pair of format string sequences: One for
                        exact-time spawns and one for time-range
                        spawns, each sequence ordered by preference.
    :rtype:             Tuple[Sequence[str], Sequence[str]]
    """
    # If any of these variables or variable pairs are used in a template,
    # then the template is displaying a time range instead of an exact time.
    rangevarsets = [{'remainrange'}, {'remainmin', 'remainmax'},
        {'untilrange'}, {'untilmin', 'untilmax'}, {'margin'}]

    now = datetime.now()
    fakeevent = Event({var: now if var.startswith('until') else 0
        for var in itertools.chain(*rangevarsets)})

    exact = []
    ranged = []

    for template in conf.TWITTER_SPAWN_FORMATS:
        try:
            _, usedvars = fakeevent.format_and_tell(template)
        except ValueError:
            raise ValueError(
                "Bad format in TWITTER_SPAWN_FORMATS: " + template)
        if any((usedvars & varset == varset for varset in rangevarsets)):
            ranged.append(template)
        else:
            exact.append(template)

    return namedtuple('TwitterSpawnFormats', 'exact ranged')(
        exact + ranged, ranged + exact)


_TWITTER_SPAWN_FORMATS = _separate_twitter_spawn_formats()
...
class TwitterSender(Sender):
    ...
    def __init__(self, *, loop, websession, config=None):
        ...
        if not _TWITTER_SPAWN_FORMATS.exact:
            raise ValueError('TWITTER_SPAWN_FORMATS is not configured')
    ...
    def _make_spawn_data(event):
        """Return data suitable for tweeting a Spawn event.
        """
        args = dict(
            lat=str(event.latitude),
            long=str(event.longitude),
            display_coordinates=True
            )

        if event.margin:
            templates = _TWITTER_SPAWN_FORMATS.ranged
        else:
            templates = _TWITTER_SPAWN_FORMATS.exact

        hashtags = _HashTagSet(event.hashtags)
        if hashtags:
            # Copy the hashtags set so we can safely discard some for brevity
            event = event.new_child(dict(hashtags=hashtags))

        # Try templates in order of preference until the text fits
...
class WebhookSender(Sender):
    ...
    def _make_spawn_data(spawn):
        ...
        fields = {
            'encounter_id':             spawn.encounterid,
            'spawnpoint_id':            spawn.spawnptid,
            'pokemon_id':               spawn.dexno,
            #'form':                     None, # not yet exposed by worker
            #'player_level':             None, # not yet exposed by worker
            'latitude':                 spawn.latitude,
            'longitude':                spawn.longitude,
            'disappear_time':           spawn.unixtime + spawn.remainmin,
            'last_modified_time':       spawn.unixtime * 1000,
...
class Channel:
        ...
        for key in species:
            try:
                if isinstance(key, str):
                    key = self._DEXNOS[key.lower()]  # look up pokemon name
            except KeyError:
                raise ValueError(
                    "config line {}: unknown pokemon {!r}".format(lineno, key))

            self.spawnrules.setdefault(key, list()).append(rule)
    ...
    def log_state(self, log):
        ...
        truedexnos = {dexno for dexno, rules in self.spawnrules.items()
            if rules == [True]}
        if truedexnos:
            log.debug("  spawns always reported: {}",
                ', '.join(names.POKEMON[n] for n in sorted(truedexnos)))
        falsedexnos = {dexno for dexno, rules in self.spawnrules.items()
            if rules == [False]}
        if falsedexnos:
            log.debug("  spawns never reported: {}",
                ', '.join(names.POKEMON[n] for n in sorted(falsedexnos)))
        otherrules = [(k, v) for k, v in self.spawnrules.items()
            if k not in truedexnos | falsedexnos | {...}]
        for dexno, rules in sorted(otherrules):
            log.debug("  {} spawn rules:", names.POKEMON[dexno])
            for rule in rules:
                log.debug("    {}", _rule_to_str(rule))
        log.debug("  fallback spawn rules:")
        for rule in self.spawnrules.get(..., []):
            log.debug("    {}", _rule_to_str(rule))
...
class ChannelMap(UserDict):  #pylint:disable=too-many-ancestors
    ...
    LegacyTests = namedtuple('LegacyTests', 'rank score time')

    def _add_legacy_rule(self, species, tests):
        """Add LegacyTests as a rule, skipping any tests that are None.
        """
        channel = self[None]
    ...
    def _add_legacy_rules(self):
        ...
        if conf.IGNORE_RARITY:
            ...
            self._add_legacy_rule(..., self.LegacyTests(
    ...
    def _test_imports(self):
        ...
        for channel in self.values():
            if not channel.spawnlaw and not channel.spawnrules:
                continue
    ...
    async def test_senders(self):
        ...
        try:
            senders = itertools.chain.from_iterable(
...
class SpawnNotifier:
    ...
    def __init__(self, channelmap):
        ...
        self.sent = 0  # number of events for which notifications have been sent
    ...
    def _make_test_vars(self, pokemon):
        """Return a dict of variables for use by tests in notification rules.
        ...
        Variables always present:
            latitude    float       Spawn location latitude
            longitude   float       Spawn location longitude
        ...
        Additional test variables can be generated by _make_time_vars().
        """
        # Variables based on spawn details
        testvars = {}
        testvars['latitude'] = pokemon['lat']
        testvars['longitude'] = pokemon['lon']
    ...
    @staticmethod
    def _make_time_vars(timestamp, remainmin=None, remainmax=None):
        """Return a dict of variables for use by time-related rules.

        :param timestamp:   POSIX timestamp of a pokemon spawn.
        ...
        Variables always present:
            ...
            unixtime    number      Unix timestamp when spawn was detected
        ...
        testvars['remain'] = remain = (remainmax + remainmin) / 2
        testvars['margin'] = (remainmax - remainmin) / 2
        testvars['until'] = when + timedelta(seconds=remain)
        testvars['untilmin'] = when + timedelta(seconds=remainmin)
        testvars['untilmax'] = when + timedelta(seconds=remainmax)
    ...
    @classmethod
    async def _diligent_make_time_vars(cls, pokemon):
        """Call _make_time_vars() using the database to estimate missing info.
        """
        ...
        return cls._make_time_vars(pokemon['seen'], *timerange)
    ...
    @classmethod
    def _make_message_vars(cls, pokemon, testvars):
        """Return a dict of variables for use by message templates.
        ...
        Variables always present:
            ...
            encounterid str         Internal ID of the pokemon spawn
            spawnptid   int or str  Internal ID of the spawn point
                                    (Type depends on conf.SPAWN_ID_INT)
        ...
        formatter = _EventFormatter()
        if testvars['margin']:
            margintext = _format_seconds(testvars['margin'])
            msgvars['remainrange'] = formatter.format(
                conf.DURATION_RANGE_FORMAT,
                min=_format_seconds(testvars['remainmin']),
                mid=_format_seconds(testvars['remain']),
                max=_format_seconds(testvars['remainmax']),
                margin=margintext)
            msgvars['untilrange'] = formatter.format(
                conf.DATETIME_RANGE_FORMAT,
                min=testvars['untilmin'],
                mid=testvars['until'],
                max=testvars['untilmax'],
                margin=margintext)
        else:
            msgvars['remainrange'] = _format_seconds(testvars['remain'])
            msgvars['untilrange'] = formatter.format_field(
                testvars['until'], '')
        ...
        if conf.LANDMARKS:
            coords = (testvars['latitude'], testvars['longitude'])
        ...
        msgvars['encounterid'] = pokemon['encounter_id']
        msgvars['spawnptid'] = pokemon['spawn_id']
    ...
    def _match_rules(self, channel, dexno, testvars, optimism=False):
        ...
        for rule in rules.get(dexno) or rules.get(...) or ():
            try:
                ...
                self.log.warning("Unexpected {!r} from {} {} spawn rule: {}",
    ...
    def eligible(self, pokemon):
        ...
        testvars.update(self._make_time_vars(
            pokemon['seen'], pokemon.get('time_till_hidden')))
    ...
    async def notify(self, pokemon, daytime):
        ...
        """
        dexno = pokemon['pokemon_id']
        name = names.POKEMON[dexno]
        encounter_id = pokemon['encounter_id']

        if encounter_id in self._handled:
            self.log.debug("Already notified: {}.{}", name, encounter_id % 1000)
            return False

        with self._handled.temp_add(encounter_id) as handling:

            testvars = self._make_test_vars(pokemon)
            testvars.update(await self._diligent_make_time_vars(pokemon))
            ...
            msgvars = self._make_message_vars(pokemon, testvars)
            ...
            self.log.debug("Notifying {} channels: {}.{}",
                len(channels), name, encounter_id % 1000)
...
class Notifier:
    ...
    async def spawn_notify(self, pokemon, daytime):
        """Send or skip a spawn notification, according to configured rules.
        See SpawnNotifier.notify() method for details.
        """
        return await self._spawnnotifier.notify(pokemon, daytime)

    def get_sent_count(self):
        """Get the number of events for which notifications have been sent.
        """
        return self._spawnnotifier.sent
