"""
Notifier configuration helpers, for use in the config file.
...
If some of these keys are omitted, the corresponding global config variables
...
When an event (like a pokemon spawn) occurs, the system consults rules
...
_SpawnArgs = namedtuple('_SpawnArgs', 'species rules lineno')
...
class _ChannelConfig:
    ...
    def __init__(self):
        ...
        self.spawnargs = []     # _SpawnArgs instances
...
class NotifyConfig:
    ...
    def spawn(self, species, *rules, channel=None):
        ...
        :param rules:   One or more spawn rules.
                        ...
                        A notification will be sent if any rule succeeds
                        (logical OR).
        ...
        if not species or isinstance(species, (str, int)):
            raise TypeError("the species arg must be a collection or ellipsis")

        # Resolving species names here is impractical because the names module
        # (as of 2017-11) requires the config to be fully loaded, but we are
        # called while it is still loading.  We therefore store our arguments
        # (along with the calling line number from the config file) and let the
        # notifier module process them later.

        lineno = inspect.stack()[1].lineno
        chanconf = self.channelconfigs.setdefault(channel, _ChannelConfig())
        chanconf.spawnargs.append(_SpawnArgs(species, rules, lineno))
    ...
    def spawn_law(self, *tests, channel=None):
        ...
        lineno = inspect.stack()[1].lineno
        chanconf = self.channelconfigs.setdefault(channel, _ChannelConfig())
        chanconf.spawnargs.append(_SpawnArgs(None, [tests], lineno))
...
# Spawn variable getters
#pylint:disable=bad-whitespace,invalid-name

latitude =  _Getter('latitude')
longitude = _Getter('longitude')
